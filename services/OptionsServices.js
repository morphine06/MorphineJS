...
module.exports = {
	get: function(co_id, name, cb) {
		Options
		.findOne({name: name, co_id: co_id})
		.exec(function(errsql, row) {
	...
	set: function(co_id, name, val, cb) {
		Options.replace({name:name, co_id:co_id}, [], {
			name:name,
			co_id:co_id,
	...
	saveAllOptions: function(co_id, opts, cb) {
		...
		async.each(optsok, function(opt, next) {
	...
	getAllOptions: function(co_id, cb) {
		...
		var opts = {} ;
		var me = this ;
		async.each(allOptions, function(optdef, next) {
