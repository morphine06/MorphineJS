		...
		if (!justCreated) {
			if (this.lastModule == module) {
				...
				for (i = 0; i < tabController.length; i++) {
					if (this.currentController.controllerName == tabController[i].controllerName)
						continue;
...
M_.Controller = class {
	...
	renderTemplate() {
		if (this.tpl) {
			this.jEl = M_.App.renderMustacheTo(
				M_.App.container,
				this.tpl,
				this.tplData,
				this.tplPartials
...
M_.Utils = class {
	...
	static downloadFile(url, args = {}, method = "POST") {
		...
		$(document.body).append(
			"<iframe id='M_FileDownload' src='about:blank' style='display:none;'></iframe>"
		);
		var html = "";
		_.each(args, (val, key) => {
	...
	static removeEventsNotAttachedToDocument() {
		window.setTimeout(() => {
			...
			for (var i = 0; i < tab.length; i++) {
				if (
					tab[i].selector &&
					$(tab[i].selector).length === 0 &&
					tab[i].selector.substring(0, 7) == "#_M_el_"
				) {
					nb++;
		...
		if (options & OPTS.PATHINFO_FILENAME) {
			...
			if (false === have_filename) {
				have_filename = have_basename.slice(
					0,
					have_basename.length -
						(have_extension
							? have_extension.length + 1
...
M_.Store = class {
	...
	load(args, callback) {
		...
		var okArgs = {};
		if (this.currentSort) {
			if (_.isArray(this.currentSort))
				okArgs.sort = this.currentSort[0] + " " + this.currentSort[1];
...
M_.Outlet = class {
	constructor(opts) {
		...
		if (!M_.Utils.isEmpty(this.jEl) && $.type(this.jEl) == "string")
			this.jEl = $("#" + this.jEl);
...
M_.Tree = class extends M_.Outlet {
			...
			this.jEl
				...
				.on("drop", evt => {
					...
					var nodeDrop = $(evt.target)
						...
						.attr("m_id");
					console.log(
						"this.isNodeParentOf(mid, nodeDrop)",
						this.isNodeParentOf(mid, nodeDrop)
					);
	...
	_createChildNodes(parent) {
		...
		html +=
			"<span><i class='" + fa + "'></i><div class='txt'>" + parent.label + "</div></span>";
	...
	_listenDocClick(evt) {
		...
		if (
			...
			return;
		this.jEl
			.find('li[m_id="' + this._currentRenameId + '"] > span > div')
			...
			this.jEl.scroll(evt => {
				M_.Utils.delay(
					() => {
						var diff =
							this.jEl.offset().top -
							this.jEl.find(".M_SimpleListContent").offset().top;
						var _skipPosition =
							Math.ceil(diff / this.lineHeight) - Math.ceil(this.loadLimit / 4);
		...
		this.jEl.find("." + this.classItems).on("mousedown", evt => {
				...
				if ((M_.Utils.isMacos && evt.metaKey) || evt.ctrlKey) {
					...
					this.store.each((row, i) => {
						if (i >= min && i <= max) {
							...
							this.jEl
								.find("." + this.classItems + '[data-m_id="' + m_idTemp + '"]')
								.addClass("selected");
							if (this.currentSelection.indexOf(m_idTemp) == -1)
								this.currentSelection.push(m_idTemp);
						}
					});
				} else {
			...
			if (this.itemsDraggableTo) {
				...
				$(document).on("mouseup", evt => {
					...
					$(document).off(
						"mouseenter",
						this.itemsDraggableTo,
						$.proxy(this.enterViewable, this)
					);
					$(document).off(
						"mouseleave",
						this.itemsDraggableTo,
						$.proxy(this.leaveViewable, this)
					);
	...
	setSelection(m_id) {
		...
		if (this.selectionVisible && goodItem.length) {
			...
			if (
				...
				1 * (posYRow - posYParent) >
					goodContainer.scrollTop() + goodContainer.height() - goodItem.height() - 30
			) {
...
M_.TableList = class extends M_.SimpleList {
	...
	create() {
		...
		html += `<div class='M_TableList ${cls}' style='${this.styleTable}'>
					...
					<div class="M_AlignRight"><a id="${this
						._idMore}" href='javascript:void(0);'>${this.getMoreText()}</a></div>
				</div>`;
		this.jEl = $(html);
		this.container.append(this.jEl);
	...
	render() {
		...
		for (var i = 0; i < colsDef.length; i++) {
			...
			html +=
				...
				colsDef[i].label;
			if (this.fullHeight)
				html += "<div style='" + style + "'>" + colsDef[i].label + "</div>";
			html += "</th>";
		}
		html += "</tr>";
		this.jEl
	...
	showOrHide(ok, anim = true) {
		...
		if (anim) {
			if (ok) {
				...
				this.jEl
					.offset({ top, left })
					.transition({ top: topStart, left: leftStart }, M_.Utils.getSpeedAnim());
			}
			// if (ok) {
	...
	showOrHide(ok, anim = true) {
		...
		if (ok) {
			...
			this.jEl
				...
				.show()
				.transition(
					{ scale: s2, opacity: o2, left: l2, top: t2, width: w2 + 1, height: h2 },
					speed,
					() => {
...
M_.Dialog = new class {
	...
	confirm(title, text, callbackOk, callbackCancel) {
		...
		html += `<div class="M_margintop">
						...
						<div class='M_FloatRight'>
							<button type="button" class='M_DialogCancel M_ModalCancel M_Button'>${this
...
M_.CalendarMonth = class {
	...
	create() {
		...
		if (!this.displayHeader)
			this.container
				.find(".M_CalendarMonth_date, .M_CalendarMonth_previous, .M_CalendarMonth_next")
				.hide();
		else
			this.container
				.find(".M_CalendarMonth_date, .M_CalendarMonth_previous, .M_CalendarMonth_next")
	...
	showYears() {
		...
		this.jEl
			.find(".M_CalendarMonth_years .M_CalendarMonth_date")
			.html(yearStart + 1 + " - " + (yearStart + 11));
	...
	showDays() {
		...
		for (i = 0; i < nbMax; i++) {
			...
			if (dActu.day() == 1) {
				...
				if (this.showWeekNumber) {
					let tdClsWeek = "";
					if (this.disabledDates && this.disabledDates(this, dActu))
						tdClsWeek = "notselectable";
					html +=
						"<td class='" +
						tdClsWeek +
						"' data-m-date='" +
			...
			if (
				...
				dActu.year() == today.year() &&
				dActu.month() == today.month() &&
				dActu.date() == today.date()
			)
				tdCls += "current ";
		...
		this.jEl
			...
			.html(
				tabMonths[this.dateViewed.month()] +
					" " +
					this.dateViewed.year() +
...
M_.Calendar.MonthView = class extends M_.CalendarMonth {
	...
	drawEvents() {
		...
		if (this.cellHeadMinHeight > 0) {
			this.jEl
				.find(".M_CalendarMonth_days thead tr")
				.css("min-height", this.cellHeadMinHeight);
		...
		for (let barEvt of tabBars) {
			var bar = $(
				"<div class='M_CalendarMonthView_bar " +
					barEvt.cls +
					"' data-m-evt='" +
					barEvt.id +
					"'>" +
...
M_.Form.Form = class {
	...
	setValues(dataOrModel) {
		...
		this.reset();
		if (!(dataOrModel instanceof M_.Model) && this.model)
			dataOrModel = new this.model({ row: dataOrModel });
	...
	getItemsByFakeGroup(fakeGroupName) {
		var keepedItems = [];
		for (var i = 0; i < this._items.length; i++) {
			if (this._items[i].fakeGroup.indexOf(fakeGroupName) >= 0)
				keepedItems.push(this._items[i]);
		}
		return keepedItems;
	}
...
M_.Form.Input = class extends M_.Outlet {
	...
	create() {
		...
		if (this.inputType == "hidden") {
			html += `<input ${readOnly} id="${this.id}" type="${this
				.inputType}" class="M_Input" name="${this.name}" value="${v}">`;
		} else {
			...
			if (this.label !== "" && (this.labelPosition == "left" || this.labelPosition == "top"))
				html += `<label style="${this.styleLabel}" class="${this
					.clsLabel}" ${forattr}>${this.label}</label>`;
			// if (this._addInputGroup) html += `<div class="M_InputGroup">` ;
			var multiple = "";
			if (this.multiple) multiple = "multiple";
			if (this.inputType == "file") {
				html += `<input ${tabindex} ${readOnly} id="${this.id}" type="${this
					.inputType}" style="${this
					.styleInput}" ${multiple} class="M_Input M_InputFile ${this
					.clsInput}" name="${this.name}" value="${v}" placeholder="${this
					.placeholder}"><label class='M_InputFileLabel' for="${this.id}">${this
					.txtChooseFile}</label>`;
			} else if (this.inputType == "textarea") {
				if (!this.height) this.height = 100;
				this.styleInput += " height:" + this.height + "px;";
				html += `<textarea ${tabindex} placeholder="${this
					.placeholder}" ${readOnly} id="${this.id}" class="M_Input ${this
					.clsInput}" style="${this.styleInput}" rows="${this.rows}" name="${this
					.name}">${v}</textarea>`;
				...
				html += `<input ${tabindex} ${readOnly} id="${this.id}" ${incremental} type="${this
					.inputType}" style="${this.styleInput}" class="M_Input ${this._clsMore} ${this
					.clsInput}" name="${this.name}" value="${v}" placeholder="${this
					.placeholder}">`;
			}
			...
			if (
				this.label !== "" &&
				(this.labelPosition == "bottom" || this.labelPosition == "right")
			)
				html += `<label style="${this.styleLabel}" class="${this
					.clsLabel}" ${forattr}">${this.label}</label>`;
			html += `</div>`;
...
M_.Form.Multi = class extends M_.Form.Input {
	constructor(opts) {
		...
		if (opts.label)
			opts.label +=
				" <span id='" +
				opts._idBtAddKeyword +
				"' class='fa fa-plus faa-pulse animated-hover'>";
		opts = opts ? opts : {};
		opts = $.extend({}, defaults, opts);
		super(opts);
...
M_.Form.Rating = class extends M_.Form.Input {
	constructor(opts) {
		...
		for (var i = 0; i < this.nbStars; i++) {
			...
			el.mouseenter({ note: i }, evt => {
				...
				// $(evt.target).addClass('over') ;
				for (var i = 0; i < this.nbStars; i++) {
					if (i <= evt.data.note * 1)
						this.jEl
							.find(".M_FormRateItem:nth-child(" + (i + 1) + ")")
							.addClass("over");
					else
						this.jEl
							.find(".M_FormRateItem:nth-child(" + (i + 1) + ")")
	...
	drawContainer() {
		...
		for (var i = 0; i < this.nbStars; i++) {
			if (i <= this.value * 1)
				this.jEl.find(".M_FormRateItem:nth-child(" + (i + 1) + ")").addClass("over");
	...
	constructor(opts) {
		...
		this.jEl.on("change", evt => {
			...
			if (this.jEl.files && this.jEl.files.length > 1)
				fileName = (this.jEl.getAttribute("data-multiple-caption") || "")
					.replace("{count}", this.jEl.files.length);
		...
		if (this.acceptTabulation) {
			this.jEl.on("keydown", e => {
				...
				if (keyCode == 9) {
					...
					this.jEl.val(
						this.jEl.val().substring(0, start) + "\t" + this.jEl.val().substring(end)
					);
...
M_.Form.TextEditor = class extends M_.Form.Textarea {
	...
	create() {
		...
		this.jEl.after(
			"<div id='" +
				idTemp1 +
				"'></div><div><div class='M_FormEditor-Content' id='" +
				idTemp2 +
				"'></div></div>"
		);
...
M_.Form.RadioGroup = class extends M_.Form.Input {
	...
	create() {
		...
		html += `<div style="${this.styleGroup}" class="M_FormGroup ${this.clsGroup}">`;
		if (this.label !== "" && (this.labelPosition == "left" || this.labelPosition == "top"))
			html += `<label style="${this.styleLabel}" class="${this.clsLabel}" ${forattr}>${this
				.label}</label>`;
		html += `<div id="${this.id}" type="${this.inputType}" style="${this
			.styleInput}" class="M_Input ${this.clsInput}">`;
...
M_.Form.Number = class extends M_.Form.Text {
	...
	setValue(v) {
		// log("setValue",v)
		if (this.startEmpty && (v === "" || v === null || typeof v === "undefined"))
			return this.jEl.val("");
		v = v * 1;
		this.value = v;
		var v2 = v;
...
M_.Form.Picker = class extends M_.Form.Text {
	...
	create() {
		...
		if (!this.hidePicker) {
			...
			this.jEl.wrap(
				"<div class='M_FormInputGroup " + moreCls + "' style='" + moreStyle + "'>"
			);
			var htmlCaret = `<div class="M_FormCaret ${this.icon}" style='${this
				.stylePicker}'></div>`;
...
M_.Form.ComboboxMulti = class extends M_.Form.Combobox {
	...
	addItemMulti(val, realval, deletable) {
		...
		var html =
			"<div class='M_ComboboxMultiItem selected' data-comboboxmultiitem=\"" +
			val +
			'" data-comboboxmultiitemval="' +
			realval +
			'">' +
			realval;
		if (deletable !== false)
			html +=
				"&nbsp;<span class='fa fa-trash faa-pulse animated-hover' id='" +
				idTemp +
				"'></span>";
		html += " </div>";
...
M_.Editor = class {
	constructor(opts) {
		var defaults = {
			...
			buttons: [
				...
				[
					"justifyFull",
					"Justifier à gauche et à droite",
					"fa-align-justify",
					"execJustify",
					"Full"
				],
	 ...
	 */
	execEditTable() {
		var table = $(document.getSelection().getRangeAt(0).commonAncestorContainer).closest(
			"table"
		);
			...
			this.winEditTable = new class extends M_.Window {
				...
				create() {
					...
					$("#" + idBtSave).click(() => {
						...
						this.currentTable.attr(
							"cellpadding",
							this.form.find("cellpadding").getValue()
						);
						this.currentTable.attr(
							"cellspacing",
							this.form.find("cellspacing").getValue()
						);
				...
				setCurrentTable(currentTable) {
					...
					if (sEl.width !== "")
						this.form.find("width").setValue(this.currentTable.get(0).style.width);
					else this.form.find("width").setValue("");
					if (this.currentTable.attr("cellpadding"))
						this.form
							.find("cellpadding")
							.setValue(this.currentTable.attr("cellpadding"));
					else this.form.find("cellpadding").setValue("");
					if (this.currentTable.attr("cellspacing"))
						this.form
							.find("cellspacing")
							.setValue(this.currentTable.attr("cellspacing"));
					else this.form.find("cellspacing").setValue("");
					if (this.currentTable.attr("border"))
						this.form.find("border").setValue(this.currentTable.attr("border"));
					else this.form.find("border").setValue("");
					if (this.currentTable.attr("align"))
						this.form.find("align").setValue(this.currentTable.attr("align"));
		...
		if (!this.winEditCell) {
			this.winEditCell = new class extends M_.Window {
				...
				setCurrentCell(currentCell) {
					...
					if (sEl.width !== "")
						this.form.find("width").setValue(this.currentCell.css("width"));
					else this.form.find("width").setValue("");
					if (this.currentCell.attr("align"))
						this.form.find("align").setValue(this.currentCell.attr("cellpadding"));
					else this.form.find("align").setValue("");
					if (this.currentCell.attr("valign"))
						this.form.find("valign").setValue(this.currentCell.attr("valign"));
					else this.form.find("valign").setValue("");
					if (sEl.backgroundColor !== "")
						this.form.find("color").setValue(this.currentCell.css("backgroundColor"));
				...
				createThumbnail(f) {
					...
					reader.onload = (function(theFile) {
						return function(e) {
							list.append(
								[
									'<img style="margin:0 15px 15px 0" src="',
									e.target.result,
									'" title="',
									theFile.name,
									'" width="100" />'
	...
	caretMove() {
		...
		if (document.queryCommandValue("justifyRight") == "true")
			this.buttonsContainer.find('button[m_id="justifyRight"]').addClass("active");
		...
		if (document.queryCommandValue("justifyCenter") == "true")
			this.buttonsContainer.find('button[m_id="justifyCenter"]').addClass("active");
		...
		if (document.queryCommandValue("justifyFull") == "true")
			this.buttonsContainer.find('button[m_id="justifyFull"]').addClass("active");
				...
				if (bt[5]) {
					button.click({ items: bt[5] }, evt => {
						...
						_.each(evt.data.items, item => {
							let nameTemp = item[1];
							if (item[2] && item[2] !== "")
								nameTemp = "<i class='fa " + item[2] + "'></i>&nbsp;" + nameTemp;
							items.push({
								text: nameTemp,
								click: evt => {
									// evt.stopPropagation() ;
	...
	setContainer(container) {
		...
		$(document).click(
			$.proxy(function(e) {
				if (
					$(e.target).closest(
						"#" + this.container.get(0).id + ", #" + this.buttonsContainer.get(0).id
					).length === 0
				) {
					$("#" + this.container.get(0).id + " .M_todelete").remove();
		...
		this.container.on(
			...
			$.proxy(function(e) {
				...
				if (!this.container.prop("contenteditable")) {
					M_.Dialog.alert(
						"Information",
						"Merci de cliquer d'abord l'endroit où vous souhaitez insérer l'image"
					);
					return;
				}
				this.container.prop("contenteditable", true);
