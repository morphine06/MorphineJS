...
nodemon.on('start', ()=> {
    console.log(chalk.green('!!!!App has started in development mode'));
    // console.log("nodemon.config",nodemon.config);
}).on('quit', ()=> {
    console.log(chalk.gray('!!!!App has quit'));
}).on('restart', (files)=> {
    console.log(chalk.gray('!!!!App restarted due to: '), files);
});
