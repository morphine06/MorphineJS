	...
	_createJoin() {
		...
		_.each(this.joinModels, (model, num) => {
			if (!model.modelnameto) tabJoin.push(M_Db.models[model.modelname].def.tableName + " " + model.modelalias);
				...
				tabJoin.push(
					"LEFT JOIN " +
						M_Db.models[model.modelname].def.tableName +
						" " +
						model.modelalias +
						" ON " +
						model.modelalias +
						"." +
						M_Db.models[model.modelname].primary +
						"=" +
						model.modelaliasto +
						"." +
						model.fieldJoin
			...
			_.each(this.def.attributes, (field, fieldName) => {
				if (field.type == "json") {
					try {
						...
						console.warn("json parse error", e, fieldName, row[fieldName]);
						row[fieldName] = null;
					}
				}
				if (field.type == "boolean") {
...
class M_Table {
	...
	replace(where, whereData, data, cb, returnCompleteRow) {
		//where, whereData,
		...
		this.findOne(where, whereData).exec((errsql, _rowold) => {
			...
			if (errsql) console.warn("errsql", errsql);
			if (!_rowold)
				this.create(data).exec((errsql, idTemp) => {
				...
				this.update(where2, whereData2, data).exec((errsql, rows) => {
...
var M_Db = new class {
			...
			query: function() {
				...
				this.pool.getConnection(function(err, connection) {
					...
					connection.query(args[0], sql_args, function(err, results) {
						...
						if (err) {
							console.warn("errrrrrr", err, args[0], sql_args);
							return callback(err);
						}
				...
				next => {
					if (this.config.migrate == "recreate") {
						...
						this.connection.query("DROP TABLE IF EXISTS " + def.tableName + "", (errsql, rows) => {
							if (errsql) console.warn("errsql2", errsql);
							next();
						});
					} else next();
				},
	...
	_getJoinedModel(field) {
		...
		} else console.warn("Model " + field.model + " not found");
		return null;
	}
}();
