...
export class Contacts extends M_.Controller {
	...
	emptyGroup() {
		...
		M_.Dialog.confirm(
			"Confirmation",
			"Etes-vous certain de vouloir vider le groupe <i>" + group.get("gr_name") + "</i><br/>Les contacts ne seront pas effacés.",
			() => {
	...
	deleteGroup() {
		...
		M_.Dialog.confirm(
			"Confirmation",
			"Etes-vous certain de vouloir supprimer le groupe <i>" + group.get("gr_name") + "</i><br/>Les contacts ne seront pas effacés.",
			() => {
	...
	showDropDownContactsSelection(evt) {
		...
		if (this.currentGroup != "-1") {
			...
			items.push({
				...
				click: () => {
					var group = this.groups.store.getRow(this.currentGroup);
					this.removeContactsToGroup(this.getContactsSelection(), group.get("gr_id"), group.get("gr_name"));
				}
			});
		}
	...
	addContactsToGroup(contacts, gr_id, gr_name) {
		M_.Utils.postJson("/1.0/groups/addcontactstogroup", { contacts: contacts, gr_id: gr_id }, data => {
			...
			M_.Dialog.notify(
				"<b>Information</b><br/>" +
					M_.Utils.plural(contacts.length, "contact ajouté", "contacts ajoutés") +
					" au groupe <i>" +
					gr_name +
					"</i>"
			);
	...
	removeContactsToGroup(contacts, gr_id, gr_name) {
		M_.Utils.postJson("/1.0/groups/removecontactstogroup", { contacts: contacts, gr_id: gr_id }, data => {
			...
			M_.Dialog.notify(
				"<b>Information</b><br/>" +
					M_.Utils.plural(contacts.length, "contact enlevé", "contacts enlevés") +
					" du groupe <i>" +
					gr_name +
					"</i>"
			);
