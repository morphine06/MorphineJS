...
module.exports = {
    checkFilesModified: (file, cb)=> {
        ...
        let lastModificationDateSaved = moment().add(-10,'years').toDate() ;
        ...
        let causedBy = '' ;
        fs.walk(morphineserver.rootDir+'/'+file)
        .on('data', (item)=> {
            // allFiles.push(item) ;
        ...
        .on('end', ()=> {
            ...
            if (lastModificationDate.getTime() >= lastModificationDateSaved.getTime()) {
                console.log("Caused by...".green + " : "+causedBy) ;
                cb(true);
            } else cb(false) ;
        }) ;
    },
    setFilesModified: ()=> {
        fs.writeJsonSync(morphineserver.rootDir+'/.tmp/.morphinejs.conf.json', {lastmodificationdate:moment().add(1,'second').format('YYYY-MM-DD HH:mm:ss')}) ;
